...
const zkConfigPath = path.resolve(new URL(import.meta.url).pathname, '..', '..', '..', 'ghost-contract', 'src', 'managed', 'ghost');
...
async function main() {
    ...
    await Rx.firstValueFrom(walletCtx.wallet.state().pipe(Rx.throttleTime(3000), Rx.tap((s) => process.stdout.write(`   synced: ${s.isSynced}\r`)), Rx.filter((s) => s.isSynced)));
            ...
            if (recipe.baseTransaction.intents) {
                for (const segment of recipe.baseTransaction.intents.keys()) {
                    ...
                    if (cloned.fallibleUnshieldedOffer) {
                        const sigs = cloned.fallibleUnshieldedOffer.inputs.map((_, i) => cloned.fallibleUnshieldedOffer.signatures.at(i) ?? sig);
                        cloned.fallibleUnshieldedOffer = cloned.fallibleUnshieldedOffer.addSignatures(sigs);
                    }
                    if (cloned.guaranteedUnshieldedOffer) {
                        const sigs = cloned.guaranteedUnshieldedOffer.inputs.map((_, i) => cloned.guaranteedUnshieldedOffer.signatures.at(i) ?? sig);
    ...
    const compiledContract = CompiledContract.make('ghost', Ghost.Contract).pipe(CompiledContract.withVacantWitnesses, CompiledContract.withCompiledFileAssets(zkConfigPath));
